...
bool TitaevMYakobiMPI::PreProcessingImpl() {
  auto& in = GetInput();
  auto& out = GetOutput();
  if (in.x0.empty()) in.x0.assign(in.n, 0.0);
  out = in.x0;
  return true;
}
...
bool TitaevMYakobiMPI::RunImpl() {
  ...
  for (int iter = 0; iter < in.max_iter; ++iter) {
    ...
    MPI_Gatherv(x_new_local.data(), my_rows, MPI_DOUBLE,
                x_new_global.data(), recvcounts.data(), displs.data(),
    ...
    if (rank == 0) {
      ...
      for (int i = 0; i < n; ++i)
        max_diff = std::max(max_diff,
                            std::fabs(x_new_global[i] - x_old[i]));
